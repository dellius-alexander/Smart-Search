...
class ConfigCertificateGenerator {
    constructor(options, certificateConfigFile, certsDir) {
        ...
        if (!certificateConfigFile.match(/.*\.req$/) || !fs.existsSync(certificateConfigFile)){
            throw new Error(`File not found: ${certificateConfigFile}. Second parameter must be a valid certificate config file with extension ["*.req"].`);
        }
    ...
    generateCertificate(
        ...
        const generateCertificate = new GenerateCertificate(domainPrefix, hostname, certsDir);
        console.log(generateCertificate.generateCertificate(hostname, certsDir, domainPrefix));
    }
