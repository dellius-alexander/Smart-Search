...
class FileCertificateGenerator extends GenerateCertificate {
    constructor(options, hostnameFile, certsDir = process.cwd()) {
        ...
        if (!hostnameFile.match(/.*\.req$/) || !fs.existsSync(hostnameFile)){
            throw new Error(`File not found: ${hostnameFile}. Second parameter must be a valid certificate config file with extension ["*.req"].`);
        }
        this.hostname = hostnameFile || false;
        if (!this.hostname) {
            throw new Error('Configuration file error: unable to locate hostname, undefined.');
        }
