...
class IntegralParser extends LatexParser {
    ...
    parseIntegral(latexDoc) {
        ...
        if (integral) {
            const lowerBound = integral[0].match(/\\int\_\{([^}]+)\}\^\{[^}]+\}\{[^}]+\}/)[1];
            const upperBound = integral[0].match(/\\int\_\{[^}]+\}\^\{([^}]+)\}\{[^}]+\}/)[1];
            const functionToIntegrate = integral[0].match(/\\int\_\{[^}]+\}\^\{[^}]+\}\{([^}]+)\}/)[1];
...
class SummationParser extends LatexParser {
    ...
    parseSummation(latexDoc) {
        ...
        if (summation) {
            const lowerBound = summation[0].match(/\\sum\_\{([^}]+)\}\^\{[^}]+\}\{[^}]+\}/)[1];
            const upperBound = summation[0].match(/\\sum\_\{[^}]+\}\^\{([^}]+)\}\{[^}]+\}/)[1];
            const functionToSum = summation[0].match(/\\sum\_\{[^}]+\}\^\{[^}]+\}\{([^}]+)\}/)[1];
...
class ProductParser extends LatexParser {
    ...
    parseProduct(latexDoc) {
        ...
        if (product) {
            const lowerBound = product[0].match(/\\prod\_\{([^}]+)\}\^\{[^}]+\}\{[^}]+\}/)[1];
            const upperBound = product[0].match(/\\prod\_\{[^}]+\}\^\{([^}]+)\}\{[^}]+\}/)[1];
            const functionToProduct = product[0].match(/\\prod\_\{[^}]+\}\^\{[^}]+\}\{([^}]+)\}/)[1];
...
class MatrixParser extends LatexParser {
    ...
    parseMatrix(latexDoc) {
        ...
        if (matrix) {
            const matrixRows = matrix[0].match(/\\begin\{matrix\}(.*)\\end\{matrix\}/s)[1].split('\\\\');
...
class VectorParser extends LatexParser {
    ...
    parseVector(latexDoc) {
        ...
        if (vector) {
            const vectorRows = vector[0].match(/\\begin\{bmatrix\}(.*)\\end\{bmatrix\}/s)[1].split('\\\\');
...
class DeterminantParser extends LatexParser {
    ...
    parseDeterminant(latexDoc) {
        ...
        if (determinant) {
            const determinantRows = determinant[0].match(/\\begin\{vmatrix\}(.*)\\end\{vmatrix\}/s)[1].split('\\\\');
...
class SetParser extends LatexParser {
    ...
    parseSet(latexDoc) {
        ...
        if (set) {
            const setRows = set[0].match(/\\begin\{Bmatrix\}(.*)\\end\{Bmatrix\}/s)[1].split('\\\\');
...
class CaseParser extends LatexParser {
    ...
    parseCase(latexDoc) {
        ...
        if (case_) {
            const caseRows = case_[0].match(/\\begin\{cases\}(.*)\\end\{cases\}/s)[1].split('\\\\');
