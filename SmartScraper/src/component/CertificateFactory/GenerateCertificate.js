...
class GenerateCertificate {
  ...
  generateCertificate(
    ...
    if (!fs.existsSync(certificateConfigFile)) {
      ...
      certificateConfigFile = CertificateConfigFileFactory.create(
        domainPrefix,
        hostname,
        certsDir
      ).generateConfig();
  ...
  generateCert(certificateConfigFile, certsDir, domainPrefix) {
    ...
    if (fs.existsSync(`${certsDir}/${domainPrefix}.req`)) {
      ...
      cert.on("exit", async () => {
        ...
        const fullChainOptions = `cat ${serverKeyFile} \
                    ${certificateFile} \
                    > \
                    ${fullChain}`;
        ...
        fullChainCmd.on("exit", async () => {
          ...
          const publicKeyOptions = `openssl \
                        x509 \
                        -pubkey \
                        -noout \
                        -in \
