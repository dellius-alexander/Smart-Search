...
class LatexParser {
   ...
   */
  getSubscriptSuperscript() {
    return this.parsedDoc.value.filter(
      (token) => token.name === "subscript" || token.name === "superscript"
    );
  }
  /**
   * getCitations
  ...
  checkSyntax(latexDoc = undefined) {
    ...
    if (this.validateCommand(latexDocText)) {
      ...
      console.log(
        `Invalid latexDocText "${latexDocText}". Please specify a valid syntax type.`
      );
      return latexDocText;
    }
  }
