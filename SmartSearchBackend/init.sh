...
PLATFORMS=${2:-$( [[ ${2} =~ (local|cordova|gh-pages|browser|ios|android|osx|add|remove|rm|backend|frontend)+ ]] && echo ${2} &2>/dev/null || echo "bad run command."; exit 1; )} # Defaults to run -- platform -- browser
...
PUBLIC_KEY_FILE=/usr/local/app/.certs/${BASE_DOMAIN}.pub
...
case ${COMMAND}-${PLATFORMS}-${OPTIONS}-${NODE_ENV}-${HTTPS} in
  run-browser-prod-${NODE_ENV}-${HTTPS})
    echo "Running ${PLATFORMS}........";
    export HTTPS=true;
    node --inspect scripts/start.js;
#    nodemon --inspect server/protocols/https.js;
    ;;
  run-browser-dev-${NODE_ENV}-${HTTPS})
    echo "Running ${PLATFORMS}........";
    nodemon --inspect ./server/protocols/http.js;
    ;;
  run-cordova-browser-production-${HTTPS})
    echo "Running ${PLATFORMS}........"
    cordova build --debug;
    #  nodemon scripts/start.js;
    cordova run ${PLATFORMS} -- --live-reload  --debug;
    ;;
  run-backend-prod-${NODE_ENV}-true)
    nodemon --inspect ./server/protocols/https.js;
    ;;
  run-backend-prod-${NODE_ENV}-false)
    nodemon --inspect ./server/protocols/http.js;
    ;;
    *)
      __help
