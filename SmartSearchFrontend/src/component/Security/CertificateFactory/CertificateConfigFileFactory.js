...
class CertificateConfigFile {
  ...
  generateConfig(
    ...
    _certDir = this.certsDir
  ) {
    try {
      console.log(
        `Attempting to generate config file for: ${hostname} @ ${_certDir}/${domainPrefix}.req`
      );
  ...
  `;
      ...
      const exampleReq = fs
        .readFileSync(`${_certDir}/${domainPrefix}.req`)
        .toString("utf-8");
      console.log(
        `Generated config file for: ${hostname} @ ${_certDir}/${domainPrefix}.req`
      );
